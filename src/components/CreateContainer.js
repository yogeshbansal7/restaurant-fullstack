  ...
  const saveDetails = () => {
    ...
    try {
      if ((!title || !calories || !imageAsset || !price || !category)) {
        ...
        setFields(true);
        setMsg('Required fiels must be filled');
        setAlertStatus('danger')
        setTimeout(() => {
          setFields(false);
          setIsLoading(false)
  ...
  return (
    <div className='w-full min-h-screen h-auto flex items-center justify-center  ' >
      <div className='w-[90%] md:w-[75%] border border-gray-300 rounded-lg p-4 flex flex-col items-center justify-center gap-4' >
        {fields && (
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className={`w-full p-2 rounded-lg text-center ${alertStatus === 'danger' ? 'bg-red-400 text-red-800' : 'bg-emerald-400 text-emerald-800'} text-lg font-semibold `} > {msg} </motion.p>
        )}
          ...
          {isLoading ? <Loader /> : <>
            {!imageAsset ? <>
              <label className='w-full h-full flex flex-col items-center justify-center cursor-pointer' >
                ...
                <input type="file" name='uploadimage' accept='images/*' onChange={uploadImage} className='w-0 h-0' />
              </label>
            </> : <>
